...
const Navbar = () => {
  ...
  return (
    ...
    >
      <div
        className={`mx-4 rounded-full px-8 py-3 flex items-center justify-between transition-all duration-300 ${scrolled ? "bg-black/80 backdrop-blur-xl border border-white/10 shadow-2xl" : "bg-black/40 backdrop-blur-lg border border-white/5"}`}
      >
        {/* Logo */}
            ...
            <div key={link.name} className="relative group">
              ...
              >
                <span className="font-medium tracking-widest text-sm uppercase flex items-center gap-1">
                  {link.name}
                  {link.isDropdown && (
                    <span className="text-[10px] mt-0.5 transition-transform duration-300 group-hover:rotate-180">
                      ▼
                    </span>
                  )}
                </span>
                <span
                  className={`absolute -bottom-1 left-0 h-[2px] bg-[#DC143C] transition-all duration-300 ${isActive(link.path) ? "w-full shadow-[0_0_10px_#DC143C]" : "w-0 group-hover:w-full"}`}
                />
              </Link>
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-black/95 backdrop-blur-xl z-40 transition-all duration-500 ease-in-out md:hidden flex flex-col items-center justify-center gap-8 ${mobileMenuOpen ? "opacity-100 translate-x-0" : "opacity-0 translate-x-full pointer-events-none"}`}
      >
        {navLinks.map((link, idx) => (
          ...
          >
            ...
            {link.isDropdown && (
              <div
                className={`w-full overflow-hidden transition-all duration-500 ease-in-out ${mobileProductsOpen ? "max-h-[600px] opacity-100 mt-4" : "max-h-0 opacity-0"}`}
              >
                <div className="bg-white/5 border border-white/10 rounded-2xl p-2 mx-4 flex flex-col gap-1 backdrop-blur-sm shadow-[inset_0_0_20px_rgba(0,0,0,0.5)]">
                  {productCategories.map((category) => (
                    ...
                    >
                      {category.title}
                      <span className="opacity-0 group-hover/subitem:opacity-100 text-[#DC143C] transition-opacity">
                        →
                      </span>
                    </button>
                  ))}
