...
const Chatbot = () => {
    ...
    const [messages, setMessages] = useState([
        { text: "Hi! I'm Datamatex AI Bot 🤖. Ask me anything about our car services!", sender: 'bot' }
    ]);
    ...
    const isHinglish = (text) => {
        const hindiKeywords = [
            "hai", "kya", "bol", "bhai", "kidhar", "kaisa", "main", "karna", "chahiye", "matlab",
            "kaun", "kaha", "kaise", "thi", "tha", "raha", "sahi", "galat", "karo", "do", "dedo",
            "wala", "wali", "kab", "kitna", "paise", "bhav", "dukan", "idhar", "udhar", "kothe",
            "malum", "samaj", "teko", "mereko", "apun", "boss", "sir", "bhaiya", "dada"
        ];
            ...
            for (const keyword of intent.keywords) {
                ...
                for (const word of inputWords) {
                    ...
                    // Fuzzy match (only for words > 3 chars to avoid noise)
                        ...
                        // Allow 1 error for words length 4-7, 2 errors for longer
                        const allowedErrors = lowerKeyword.length > 7 ? 2 : 1;
    ...
    return (
        <div className="fixed bottom-6 right-6 z-[100] font-sans flex flex-col items-end gap-2">
            ...
            {/* Greeting Popup */}
            <div className={`transition-all duration-500 transform ${showPopup && !isOpen ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-10 pointer-events-none'} bg-white text-black text-sm px-4 py-3 rounded-xl shadow-lg border-2 border-[#E31E24] relative mb-2 max-w-[200px]`}>
            ...
            >
                {/* Header */}
                <div className="bg-gradient-to-r from-[#E31E24] to-[#990a0f] p-4 flex items-center justify-between shadow-md">
                    <div className="flex items-center gap-3">
                        <div className="w-10 h-10 bg-black/20 rounded-full flex items-center justify-center p-1 border border-white/20 backdrop-blur-sm">
                            <svg className="w-6 h-6 fill-white" viewBox="0 0 24 24"><path d="M12 2a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 0 1 7 7h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-1v1a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-1H2a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1h1a7 7 0 0 1 7-7V5.73C9.4 5.39 9 4.74 9 4a2 2 0 0 1 2-2M7.5 13A2.5 2.5 0 0 0 5 15.5A2.5 2.5 0 0 0 7.5 18A2.5 2.5 0 0 0 10 15.5A2.5 2.5 0 0 0 7.5 13m9 0a2.5 2.5 0 0 0-2.5 2.5a2.5 2.5 0 0 0 2.5 2.5a2.5 2.5 0 0 0 2.5-2.5a2.5 2.5 0 0 0-2.5-2.5M12 11h-2v1h4v-1h-2Z" /></svg>
                        </div>
                        <div>
                            <h3 className="font-black text-white uppercase italic tracking-wider text-sm">Datamatex AI Bot</h3>
                            <p className="text-white/80 text-[10px] uppercase tracking-widest font-bold flex items-center gap-1">
                                <span className="w-1.5 h-1.5 bg-green-400 rounded-full animate-pulse"></span> Online
                            </p>
                        </div>
                    </div>
                    <button onClick={toggleChat} className="text-white/80 hover:text-white transition-colors rotate-0 hover:rotate-90 duration-300">
                    ...
                    {messages.map((msg, idx) => (
                        <div key={idx} className={`flex ${msg.sender === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-2 duration-300`}>
                            {msg.sender === 'bot' && idx > 0 && (
                                <div className="w-8 h-8 rounded-full bg-[#E31E24]/20 flex items-center justify-center mr-2 border border-[#E31E24]/30 self-end mb-1 shrink-0">
                                    <svg className="w-4 h-4 fill-white" viewBox="0 0 24 24"><path d="M12 2a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 0 1 7 7h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-1v1a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-1H2a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1h1a7 7 0 0 1 7-7V5.73C9.4 5.39 9 4.74 9 4a2 2 0 0 1 2-2M7.5 13A2.5 2.5 0 0 0 5 15.5A2.5 2.5 0 0 0 7.5 18A2.5 2.5 0 0 0 10 15.5A2.5 2.5 0 0 0 7.5 13m9 0a2.5 2.5 0 0 0-2.5 2.5a2.5 2.5 0 0 0 2.5 2.5a2.5 2.5 0 0 0 2.5-2.5a2.5 2.5 0 0 0-2.5-2.5M12 11h-2v1h4v-1h-2Z" /></svg>
                                </div>
                            )}
                            <div
                ...
                <div className="bg-[#0a0a0a] border-t border-white/10">
                    ...
                    <form onSubmit={handleSendMessage} className="p-3 flex gap-2">
                        ...
                        >
                            <svg className="w-5 h-5 fill-current transform rotate-90" viewBox="0 0 24 24"><path d="M2.01 21L23 12 2.01 3 2 10l15 2-15 2z" /></svg>
                        </button>
                    </form>
                </div>
            </div>
            ...
            {!isOpen && (
                ...
                >
                    ...
                    <svg className="w-8 h-8 fill-white" viewBox="0 0 24 24"><path d="M12 2a2 2 0 0 1 2 2c0 .74-.4 1.39-1 1.73V7h1a7 7 0 0 1 7 7h1a1 1 0 0 1 1 1v3a1 1 0 0 1-1 1h-1v1a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-1H2a1 1 0 0 1-1-1v-3a1 1 0 0 1 1-1h1a7 7 0 0 1 7-7V5.73C9.4 5.39 9 4.74 9 4a2 2 0 0 1 2-2M7.5 13A2.5 2.5 0 0 0 5 15.5A2.5 2.5 0 0 0 7.5 18A2.5 2.5 0 0 0 10 15.5A2.5 2.5 0 0 0 7.5 13m9 0a2.5 2.5 0 0 0-2.5 2.5a2.5 2.5 0 0 0 2.5 2.5a2.5 2.5 0 0 0 2.5-2.5a2.5 2.5 0 0 0-2.5-2.5M12 11h-2v1h4v-1h-2Z" /></svg>
